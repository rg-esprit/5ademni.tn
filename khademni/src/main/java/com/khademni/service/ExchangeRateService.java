 ...
 * - Méthode utilitaire convertPrice(amount, currency)
 */
public class ExchangeRateService {

    private static final String API_KEY = "565b6f59db3f866cc1e8fd5f";
    ...
    public String formatConversions(double amountTND) {
        ...
        return String.format("≈ %.2f € / %.2f $", eur, usd);
    }
    ...
    private void loadFromCache() {
        try {
            ...
            if (json != null && (System.currentTimeMillis() - timestamp) < CACHE_TTL_MS) {
                parseRatesJson(json);
                loaded = true;
                System.out.println("[ExchangeRate] Taux chargés depuis le cache local.");
            }
        } catch (Exception e) {
    ...
    private void saveToCache(String json) {
        try {
            ...
            prefs.flush();
            System.out.println("[ExchangeRate] Taux sauvegardés dans le cache local.");
        } catch (Exception e) {
        ...
        Thread thread = new Thread(() -> {
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    saveToCache(body);
                    loaded = true;
                    System.out.println("[ExchangeRate] Taux rafraîchis depuis l'API. EUR="
